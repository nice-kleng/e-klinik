...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0"><i class="fas fa-pills me-1"></i>Antrian Farmasi</h4>
    ...
    <div class="card border-0 shadow-sm mb-3 border-start border-4 border-primary">
        <div class="card-body d-flex justify-content-between align-items-center">
            ...
            <form method="POST" action="{{ route('pharmacy-queues.complete', $current) }}"
        ...
        <div class="card-body p-0">
            ...
            <table class="table table-striped mb-0">
                ...
                <tbody>
                        ...
                        <tr class="{{ $q->status == 'completed' ? 'opacity-50' : '' }}">
                            ...
                            <td>
                                @if(in_array($q->status, ['called', 'in_progress']))
                                    <form method="POST" action="{{ route('pharmacy-queues.complete', $q) }}" class="d-inline"
                                          onsubmit="return confirm('Tandai antrian ini selesai?')">
                                        @csrf
                                ...
                                @if(in_array($q->status, ['waiting', 'called']))
                                    <form method="POST" action="{{ route('pharmacy-queues.cancel', $q) }}" class="d-inline"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">Tidak ada antrian farmasi</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="callModal" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('pharmacy-queues.call-next') }}" class="modal-content">
            @csrf
