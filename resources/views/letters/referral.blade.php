...
<body>
    ...
    <p style="text-align:center">Nomor: SR/{{ $registration->registration_number }}</p>

    <div class="content">
        <p>Kepada Yth.<br>
        <strong>{{ $summary->referral_to ?? 'Faskes Tujuan' }}</strong><br>
        di Tempat</p>

        <p>Dengan hormat,</p>

        <p>Bersama ini kami merujuk pasien:</p>
        ...
        @if($mr)
        <p>Diagnosis: <strong>{{ $mr->diagnoses->first()?->icd10Diagnosis->code ?? $mr->diagnosis_primary ?? '-' }}</strong></p>
        @endif

        <p>Untuk selanjutnya dapat dilakukan pemeriksaan dan penanganan lebih lanjut.</p>

        @if($summary?->referral_notes)
            <p>Catatan: {{ $summary->referral_notes }}</p>
        @endif
